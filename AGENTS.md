...
- **#428 Whitney docs notes** — the scalar-shape bug itself is fixed (PR #430: reshape to the input shape then `_scalarize`, contract pinned across every `TemperatureInterpolator` in `tests/regression/test_issue_428.py`). Still open from the same report: document that Whitney's `f(T_b) + f'(T_b)(T - T_b)` extension is a *constant-entropy* continuation — the reason to reach for it over `PolyFit` for `F(T)` — and that its exact interpolation makes a residual-against-own-input check useless as a fit-quality gate.
