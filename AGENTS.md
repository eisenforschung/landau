...
| `landau/plot.py` | `plot_phase_diagram`, `plot_mu_phase_diagram`, 1d variants, `plot_excess_free_energy`, `get_polygons`. Both 2d plotters take `transition_temperatures=False` (labels every `TRIPLE`/`CONGRUENT` invariant with its temperature; needs a refined frame) and `triplepoints=None` (sentinel: marks follow `transition_temperatures` unless set explicitly — PR #452) |
