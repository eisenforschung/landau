...
| `landau/features.py` | `Locus` `StrEnum` (`INTERIOR`/`BOUNDARY`/`TRIPLE`/`CONGRUENT`) backing the `locus` column of `calc_phase_diagram` output; imported as `from landau.features import Locus` (not re-exported from package root) |
...
- **`distance_threshold`** is a required kwarg on `cluster_T_c` / `cluster_T_c_mu` / `cluster`; user-facing `cluster_phase` and `get_polygons` default it to `0.2` (lowered from 0.5 in PR #461, issue #456).
...
- **#116 refactor umbrella** — long-running sweep splitting big functions and pinning private helpers with direct unit tests. Sub-issues open at the time of writing (check before duplicating):
  ...
  - #425 (`_label_fits`, now in `plot/labels.py`)
...
- **#456 disjoint stable fields of one phase drawn as a single bridged polygon** — closed 2026-09-08 by PR #461: the old default `distance_threshold=0.5` merged same-phase stable fields far apart in c at the same T, after which every `poly_method` bridged the gap (`concave` silently — worse). Default lowered to `0.2` (safe band on the fixtures ~0.01–0.3; over-splitting starts between 0.05 and 0.1); the Y-Zn liquid frame is a gzip fixture in `tests/regression/test_issue_456.py`.

- **#463, #464 transition-temperature annotation follow-ups** (filed 2026-09-07 against 1.14.0). #464: one congruent maximum tagged `CONGRUENT` twice (adjacent narrow trace points both pass the relative-gap criterion) plus terminal melting/boiling points tagged congruent — duplicate labels. #463: obstacle avoidance can drift labels too far to attribute. PR #468 (merged 2026-09-08) landed the groundwork: `plot.py` → `landau/plot/` subpackage, label placement in `plot/labels.py`, behaviour-preserving. In flight on top: PR #474 (the placement half, refs #463 — scores every candidate offset by cost instead of taking the first clear one; duplicate invariants within one label share a single label), PR #471 (the tagging half — solves congruent points exactly at the fold and adds a `TerminalRefiner` for pure-component transitions at `mu = ±inf`; stacked on #467), and PR #478 (stacked on #471, opened 2026-09-09 — tags the closure of a miscibility gap, i.e. the consolute point, as congruent: `MiscibilityGapRefiner._tag_features` bisects the closure in T via an exact `_gap_jump` read through the phase's own `concentration`, emits a `RefinedMiscibilityGap` with both branches at the critical composition and a new `congruent` field, and drops the trace's overshoot past it).
...
- **Open PRs in flight** (check before duplicating): #462 (TDB export), #490 (direct tests for the LTE saturation clamp, closes #460), the #467 → #471 → #478 → #465 stack (monotectic-type invariant tagging via a `_dominated_node` hook on the gap tracer; exact congruent-point solve + `TerminalRefiner`; miscibility-gap closure tagged as its congruent point; TransitionTemperatures notebook — the stack's base, #466's seed-slope CC bootstrap bracket, merged 2026-09-15), #474 (cost-based transition-temperature label placement, refs #463), #487 (release-please 1.14.2 — carries #466), #496 (a `dphi = phi - min(phi)` metastability-margin column on `keep_unstable=True` frames — item 4 of #337), #497 (`RedlichKister.fit` by `numpy.linalg.lstsq` instead of `curve_fit`; under-determined fits become minimum-norm, terminals-only fits return the chord instead of raising), the CEF stack #324/#326/#346 (+ parked #334) behind #344, and long-open design prototypes #306 (`IntermetallicPhase`), #250 (`PhaseDiagram` object interface), #249 (`BufferedSegments`).
