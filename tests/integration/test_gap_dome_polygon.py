...
plus the compound. ``get_transitions`` splits it into its two tie-lines, so the
liquid's branches end the dome and the liquidus instead of forming a chord
segment of their own across the two-phase field, which the stitch would take
in place of the dome (reported on the TransitionTemperatures notebook).
"""
import numpy as np
import pytest
import shapely
