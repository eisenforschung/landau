...
- **#116 Refactor Opportunities umbrella** — recent sub-issues:
  ...
  - #171: drop unused `eps=0.01` kwarg from `cluster_T_c` / `cluster_T_c_mu` / `cluster` (docstrings already mark it unused — `distance_threshold` is the only live control).
  - #172 / PR #174 (open): `handle_poly_method('concave', ratio=0.3)` raises `TypeError` because `ratio` is left in `**kwargs` while also passed explicitly; PR #174 pops it through `kwargs.pop('alpha', kwargs.pop('ratio', Concave.ratio))`.
  - #173: pin the `handle_poly_method(None, ...)` default fall-through chain (`segment-fasttsp → segment-tsp → fasttsp → tsp → concave`).
  - PR #170 (open): direct unit tests for `_split_stable`, `_border_edges`, `reduce`, and the `cluster` dispatcher in `calculate.py` so the upcoming kwarg/refactor passes above are safe.
