...
- `plot_phase_diagram` (c–T), `plot_mu_phase_diagram` (μ–T), `plot_1d_mu_phase_diagram`, `plot_1d_T_phase_diagram`. The mu/c variants share `_plot_phase_diagram` + the `_set_axis_for` helper (PR #122). Generalising the 1D/2D split is open (#34, #60). `plot_1d_T_phase_diagram` assigns per-segment `_seg_id`s by running `cluster_T_c` per `(phase, stable)` group with a data-informed threshold (`1.5 × max-normed-T-gap` over the stable groups) so a phase unstable in two disjoint T windows draws as separate dashed segments instead of one line through the stable region (commits e310127/bac93cf/3e45ad0; #177 still open with leftover artifacts).
