...
- `basic.py` — `Interpolator` (ABC), `TemperatureInterpolator`, `ConcentrationInterpolator`, `PolyFit`, `SplineFit` (`ConcentrationInterpolator` wrapping `scipy.interpolate.UnivariateSpline`; `smoothing=0` strict interpolation, `>0` smoothing, `None` scipy default; `degree` clamped to `len(x)-1`; extrapolation extends the edge segment per scipy's default `ext=0`), `SGTE`, `RedlichKister` (+ `RedlichKisterInterpolation` helper), `StitchedFit`, `G_calphad` standalone fn. Also home to the `_scalarize(x)` 0-d→Python-scalar helper (relocated here from `landau.phases` so both layers share one copy; `landau.phases` re-imports it).
...
- **Refiner extensions** — `ClausiusClapeyronRefiner` and `MiscibilityGapRefiner` are on by default for 2-D grids; `DelaunayLineRefiner` and the 1-D `ScanRefiner` remain opt-in. Open bug: not all triple points are caught with `triplepoints=True` (#32, reproducer in the Intermetallics notebook; PR #247 renamed the keyword from `tielines=`).
...
- **#116 Refactor Opportunities umbrella** — open sub-issues:
  ...
  - #210 (move `DelaunayTripleRefiner` dedup state out of `solve()`): PR #256 attempted the thin-`run()`-override route and merged; PR #259 (open) follows up with a cleaner "attribute triple points to owning simplex via barycentric containment" approach that supersedes #256's mechanism.
...
- `DelaunayTripleRefiner` dedup state moved out of `solve()` into a thin `run()` override (PR #256, partial #210) — restores the pure-`solve()` convention every other refiner follows; the alternative simplex-attribution approach is tracked in open PR #259.
