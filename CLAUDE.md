...
- Private `_apply_series(grouped, fn, name)` (PR #242) wraps the pandas 2/3 single-group `groupby().apply()` workaround: runs `fn(g).to_frame(name)` per group and indexes back out, so a `Series`-returning callable composes consistently across pandas versions. Used by `calc_phase_diagram` (`f_excess`) and `get_transitions` (`transition_unit`); `plot.py::cluster_phase` still carries an inline copy (#244 open follow-up).
...
- **#116 Refactor Opportunities umbrella** — open sub-issues:
  ...
  - #244: hoist `_apply_series` out of `calculate.py` so `plot.py::cluster_phase` (still carrying an inline copy of the pandas 2/3 single-group `groupby.apply` workaround) can share it (follow-up to PR #242).
  - #245: direct unit tests for `calc_phase_diagram::sub` (the tangent-endpoint `f_excess` reference from PR #231) — current coverage is whole-pipeline only via `test_excess_endpoint_reference.py`.
  ...
  - #160 closed by `dac7d73`/PR #242; #173 closed by PR #224; #211 closed by PR #236; #243 closed by PR #251 (see landed list).
