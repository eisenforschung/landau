...
- **Excess-free-energy / common-tangent plot** (#136) — `plot_excess_free_energy` + helpers (`_phase_concentration_range`, `_endmember_free_energy`, `_lower_convex_hull`) under review in PR #145 (ready, base branch `claude/issue-136-20260521-1020`); PR #146 (tests) already merged into that branch. Touches `landau/plot.py` only. Bug #155 (`f_excess` reference picks wrong endpoint phase when phases overlap near c=0/c=1) was identified during this work and still needs a fix.
