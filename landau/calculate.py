...
def find_mu_one_point(phase1, phase2, mu_range, T):
    ...
    mu = find_one_point(
            phase1, phase2,
            lambda p, mu: p.semigrand_potential(T, mu), mu_range
    )
    return [{
        'mu': mu,
        'phi': phase1.semigrand_potential(T, mu),
        'c': phase1.concentration(T, mu),
        'phase': phase1.name
    }, {
        'mu': mu,
        'phi': phase2.semigrand_potential(T, mu),
        'c': phase2.concentration(T, mu),
        'phase': phase2.name
    }]
...
def find_T_one_point(phase1, phase2, T_range, mu):
    T = find_one_point(
            phase1, phase2,
            lambda p, T: p.semigrand_potential(T, mu), T_range
    )
    return [{
        'T': T,
        'phi': phase1.semigrand_potential(T, mu),
...
def find_all_points(stable_df, phases, by='mu'):
    ...
    if by == 'mu':
        boundaries = [
                    {'mu': -np.inf,
                    'c': 0,
                    'phase': stable_df.phase.iloc[0]},
                    {'mu': np.inf,
                    'c': 1,
                    'phase': stable_df.phase.iloc[-1]},
        ]
    else:
        boundaries = [
                    {'T': stable_df['T'].min(),
                    'c': stable_df.c.iloc[0],
                    'phase': stable_df.phase.iloc[0]},
                    {'T': stable_df['T'].max(),
                    'c': stable_df.c.iloc[-1],
                    'phase': stable_df.phase.iloc[-1]},
        ]
...
def refine_phase_diagram(df, phases, min_c=0, max_c=1):
    ...
    if multiple_mus and multiple_ts:
        ...
        if multiple_mus:
            mdf = df.groupby('T', group_keys=True).apply(
                    find_all_points, phases=phases, by='mu',
                    include_groups=True
            ).reset_index()
            mdf['stable'] = True
            mdf['border'] = True
        ...
        if multiple_ts:
            Tdf = df.groupby('mu', group_keys=True).apply(
                    find_all_points, phases=phases, by='T',
                    include_groups=True
            ).reset_index()
            Tdf['stable'] = True
            Tdf['border'] = True
...
def get_transitions(df):
    ...
    tdf['border_segment'] = tdf[['transition', 'transition_unit']].apply(lambda r: '_'.join(map(str, r.tolist())), axis='columns')
