...
def calc_phase_diagram(
    ...
    Returns:
        ...
        ``"boundary"``, ``"triple"`` or ``"congruent"``); the ``dphi``
        column is ``phi`` minus the lowest ``phi`` among the rows at the
        same ``(T, mu)``, i.e. how far a phase sits above the stable one
        (``0`` for stable rows, ``NaN`` on the synthetic ``mu = +-inf``
        edges). Refined rows only carry their coexisting phases, so there
        it measures the spread between those.
    """
